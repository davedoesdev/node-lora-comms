...
class LinkDuplex extends Duplex
{
    ...
    {
        send_to(this._link, data, -1, -1, -1, (err, r) =>
        {
            if (err)
            {
    ...
    _read()
    {
        ...
        recv_from(this._link, buf, -1, -1, (err, r) =>
        {
            if (err)
            {
                if (err.errno === LoRaComms.EBADF)
...
module.exports = new class extends EventEmitter
{
    ...
    start(options)
    {
        ...
        this.uplink = new CommsDuplex(LoRaComms.uplink,
                                      finish_end,
                                      options);

        this.downlink = new CommsDuplex(LoRaComms.downlink,
                                        finish_end,
                                        options);
